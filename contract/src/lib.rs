...
#[contract]
pub struct RemitStudyVault;

#[contractimpl]
impl RemitStudyVault {
    ...
    pub fn open_student_vault(env: Env, id: String, owner: Address, target: i128) -> Result<PaymentRecord, ContractError> {
        if target <= 0 {
    ...
    pub fn fund_tuition_escrow(env: Env, id: String, from: Address, amount: i128) -> Result<i128, ContractError> {
        if amount <= 0 {
            return Err(ContractError::InvalidAmount);
        }
        from.require_auth();
    ...
    pub fn mark_school_progress(env: Env, id: String, score: u32, status: String) -> Result<PaymentRecord, ContractError> {
    ...
    pub fn release_tuition(env: Env, id: String, to: Address, amount: i128) -> Result<i128, ContractError> {
        if amount <= 0 {
            return Err(ContractError::InvalidAmount);
        }
        Self::require_admin(&env);
...
mod test {
    ...
    fn constructor_sets_project_name() {
        ...
        let name = String::from_str(&env, "RemitStudy Vault");
        let contract_id = env.register(RemitStudyVault, (admin, asset, name.clone()));
        let client = RemitStudyVaultClient::new(&env, &contract_id);
    ...
    fn creates_domain_record() {
        ...
        let asset = Address::generate(&env);
        let name = String::from_str(&env, "RemitStudy Vault");
        let contract_id = env.register(RemitStudyVault, (admin, asset, name));
        let client = RemitStudyVaultClient::new(&env, &contract_id);

        let owner = Address::generate(&env);
        let id = String::from_str(&env, "record-001");
        let record = client.open_student_vault(&id, &owner, &1000);
